...
684 clusters (3230 total sites) across `src/` and `tests/`, found by `tests/simplification_audit.rs`'s deterministic normalized-token-shingle Jaccard pass (8-token shingles, threshold 0.72) plus five mandatory mechanical sweeps. Strict definition (spec 85 Goal): any logic present in more than one place anywhere in the codebase is a violation, with no "small enough to duplicate" exemption.
...
Recall check (spec 85 THOROUGHNESS): 30 functions drawn by seeded random index (seed `85072026`, `sample_indices` over all 5959 functions scanned in `src/` and `tests/`, excluding `tests/prioritized_plan_citation_periphery.rs` - criterion 4's own citation-guard periphery test, whose function count grows as its citation-drift-guard mechanism hardens round over round; excluding it keeps that unrelated growth from ever reshuffling this already-verified draw), each read by hand - together with its host file's surrounding context, since a duplicate can live anywhere in the file or a sibling file - to judge whether a duplicate exists that the mechanical pass and the five sweeps above did not already catch.

- `src/conductor.rs:2560-2566` `agent_model` - no duplicate found by reading
- `src/conductor.rs:3539-3561` `log_review_tier` - no duplicate found by reading
- `src/conductor.rs:13203-13275` `a_stale_non_matching_id_falls_back_to_the_verbatim_prose_match` - caught: `dup-0050`
- `src/conductor.rs:16692-16714` `render_capped_lessons_scoped` - caught: `dup-0052`
- `src/config.rs:2389-2396` `unbounded_wall_clock_advisory_is_silent_when_the_default_is_bounded` - no duplicate found by reading
- `src/contextgraph/sqlite.rs:6053-6126` `an_upheld_finding_expires_when_its_unit_integrates_but_stays_live_until_then` - no duplicate found by reading
- `src/dash.rs:8297-8317` `chain_graph_local` - caught: `dup-0137`
- `src/eventstore/mod.rs:773-790` `endpoint_label` - no duplicate found by reading
- `src/main.rs:15860-15886` `current_run_units_still_retires_a_terminal_unit_once_its_latest_spawn_answers` - no duplicate found by reading
- `src/mcpserver.rs:1009-1074` `peers_json_core_matches_the_mcp_tool` - no duplicate found by reading
- `src/registry.rs:133-135` `instances_dir` - caught: `dup-0284`
- `src/run.rs:1097-1106` `effective_definition_folds_the_last_rebase_over_the_pinned_start` - no duplicate found by reading
- `src/spawn.rs:338-341` `with_system_prompt` - caught: `dup-0294`
- `src/spec.rs:121-135` `checkbox_text` - no duplicate found by reading
- `src/worktree.rs:1067-1124` `sweep_terminal_logged` - no duplicate found by reading
- `src/worktree.rs:2504-2545` `worktree_remove_reclaims_the_sibling_per_unit_cache` - caught: `dup-0336`
- `tests/cli.rs:1372-1389` `prompt_refuses_to_fabricate_a_store_when_none_exists` - caught: `dup-0391`
- `tests/cli.rs:11473-11490` `write_reviewed_workflow_no_gate` - caught: `dup-0418`
- `tests/cli.rs:26478-26498` `cmd_dash_leaves_a_non_addrinuse_bind_error_unenriched` - no duplicate found by reading
- `tests/concepts_derivation_cli.rs:83-92` `concepts` - caught: `dup-0482`
- `tests/graph_density_spread_floor_and_centring.rs:184-199` `the_spread_factor_floors_at_one_off_the_dense_path` - caught: `dup-0544`
- `tests/mutation_scratch_persona_encoding_contract_periphery.rs:37-41` `read_persona` - caught: `dup-0344`
- `tests/no_os_kill_test_helper_periphery.rs:213-218` `stop_pid_refuses_pid_zero` - caught: `dup-0585`
- `tests/published_content_key_split_periphery.rs:72-74` `published_policy` - no duplicate found by reading
- `tests/rationale_overlay_seam.rs:135-139` `body_of` - caught: `dup-0520`
- `tests/reap_before_removal_audit.rs:1522-1541` `an_exemption_marker_after_the_removal_still_covers_it` - caught: `dup-0593`
...
- `tests/subject_lens_defined_cells.rs:255-272` `mixed_membership_graph` - caught: `dup-0670`
