...
771 clusters (3707 total sites) across `src/` and `tests/`, found by `tests/simplification_audit.rs`'s deterministic normalized-token-shingle Jaccard pass (8-token shingles, threshold 0.72) plus five mandatory mechanical sweeps. Strict definition (spec 85 Goal): any logic present in more than one place anywhere in the codebase is a violation, with no "small enough to duplicate" exemption.
...
### Clusters (247 exact, 444 near, 80 semantic)
...
- `tests/cli.rs:29061-29061` `".rigger"`
...
- `tests/cli.rs:29128-29168` `docs_installs_the_operator_lookup_rule_text_into_the_shipped_skill_and_handbook`
...
- `tests/cli.rs:28880-28985` `mcp_survives_a_grounder_resolution_failure_and_still_serves_peers_and_graph`
...
Recall check (spec 85 THOROUGHNESS): 30 functions drawn by seeded random index (seed `85072026`, `sample_indices` over all 6706 functions scanned in `src/` and `tests/`, excluding `tests/prioritized_plan_citation_periphery.rs` - criterion 4's own citation-guard periphery test, whose function count grows as its citation-drift-guard mechanism hardens round over round; excluding it keeps that unrelated growth from ever reshuffling this already-verified draw), each read by hand - together with its host file's surrounding context, since a duplicate can live anywhere in the file or a sibling file - to judge whether a duplicate exists that the mechanical pass and the five sweeps above did not already catch.

- `src/concepts.rs:383-385` `membership` - caught: `dup-0021`
- `src/conductor.rs:18036-18071` `the_verbatim_count_cap_binds_on_many_small_findings` - no duplicate found by reading
- `src/conductor.rs:28547-28594` `nine_of_ten_budget_crosses_the_final_tenth` - no duplicate found by reading
- `src/dash.rs:10122-10176` `calls_route_runs_the_traversal_for_view_calls_and_declines_otherwise` - no duplicate found by reading
- `src/driver/replay.rs:1268-1275` `config_with` - no duplicate found by reading
- `src/eventstore/kurrentdb.rs:698-733` `passes_the_contract` - no duplicate found by reading
- `src/eventstore/mod.rs:1030-1039` `an_empty_report_is_the_same_failure_and_never_a_position` - no duplicate found by reading
- `src/eventstore/namespace.rs:97-106` `append` - no duplicate found by reading
- `src/gate.rs:1836-1849` `resolve_wrapper_name_named_wrapper_absent_from_path_errors_naming_the_binary` - no duplicate found by reading
- `src/ledger.rs:182-191` `attention_kind_rank` - no duplicate found by reading
- `src/main.rs:4378-4383` `derive_extent_end` - no duplicate found by reading
- `src/main.rs:18596-18618` `require_store_dir_fence_is_off_by_default` - no duplicate found by reading
- `src/main.rs:19987-20067` `migrate_project_identity_rekeys_the_graph_before_the_irreversible_stream_rename` - no duplicate found by reading
- `src/main.rs:21576-21595` `meta_object_body` - no duplicate found by reading
- `src/main.rs:23714-23729` `assert_advisory_for_never_fabricates_a_missing_store` - no duplicate found by reading
- `src/mcpserver.rs:1239-1286` `emit_tool_folds_a_review_finding_into_the_wired_graph` - no duplicate found by reading
- `src/watch.rs:77-120` `order_signatures` - no duplicate found by reading
- `tests/adaptive_labels_periphery.rs:95-110` `run_node` - no duplicate found by reading
- `tests/cli.rs:21680-21739` `release_ready_handoff_surfaces_on_status_for_a_done_run` - caught: `dup-0416`
- `tests/code_entity_test_exclusion_periphery.rs:1211-1254` `a_trailing_comma_in_a_wrapped_cfg_predicate_still_excludes_the_module_through_the_public_api` - caught: `dup-0510`
- `tests/files_lens_view_periphery.rs:91-97` `plain` - caught: `dup-0522`
- `tests/no_os_kill_audit.rs:180-182` `shape_libc_kill` - caught: `dup-0648`
- `tests/no_os_kill_audit.rs:324-337` `collect_rs_files` - caught: `dup-0597`
...
- `tests/reset_derived_compaction_periphery.rs:3287-3319` `seed_project_under_the_legacy_namespace` - no duplicate found by reading
- `tests/simplification_audit.rs:775-851` `skip_string_literal` - no duplicate found by reading
- `tests/simplification_audit.rs:7035-7049` `section_1_reports_none_unassigned_explicitly_when_everything_is_assigned` - caught: `dup-0707`
...
- `tests/subject_lens_reprojection_contract.rs:297-328` `reprojection_carries_empty_state_when_no_member_realizes_any_concept_under_the_concepts_lens` - caught: `dup-0763`
