...
767 clusters (3703 total sites) across `src/` and `tests/`, found by `tests/simplification_audit.rs`'s deterministic normalized-token-shingle Jaccard pass (8-token shingles, threshold 0.72) plus five mandatory mechanical sweeps. Strict definition (spec 85 Goal): any logic present in more than one place anywhere in the codebase is a violation, with no "small enough to duplicate" exemption.

### Mandatory sweeps

- **Command::new call sites**: 376 site(s) - `dup-0006`
- **/proc-path string literals**: 60 site(s) - `dup-0145`
- **sqlite Connection::open call sites**: 46 site(s) - `dup-0123`
- **.rigger-path string literals**: 705 site(s) - `dup-0061`
- **error-shaping helper functions**: 8 site(s) - `dup-0080`

### Clusters (245 exact, 440 near, 82 semantic)
...
mandatory sweep: Command::new call sites - 376 site(s), collected mechanically regardless of the Jaccard pass (spec 85 Design)
...
#### `dup-0061` (semantic, 705 sites)

Proposed home: `one .rigger-relative path-composition helper`

mandatory sweep: .rigger-path string literals - 705 site(s), collected mechanically regardless of the Jaccard pass (spec 85 Design)
...
Recall check (spec 85 THOROUGHNESS): 30 functions drawn by seeded random index (seed `85072026`, `sample_indices` over all 6668 functions scanned in `src/` and `tests/`, excluding `tests/prioritized_plan_citation_periphery.rs` - criterion 4's own citation-guard periphery test, whose function count grows as its citation-drift-guard mechanism hardens round over round; excluding it keeps that unrelated growth from ever reshuffling this already-verified draw), each read by hand - together with its host file's surrounding context, since a duplicate can live anywhere in the file or a sibling file - to judge whether a duplicate exists that the mechanical pass and the five sweeps above did not already catch.

- `src/conductor.rs:10332-10354` `review_only_worktree` - no duplicate found by reading
- `src/conductor.rs:23867-23886` `route_review_tier_routes_a_low_risk_unit_to_the_light_panel` - no duplicate found by reading
- `src/conductor.rs:29446-29495` `the_adjudicators_spawn_is_stamped_with_lenses_plus_adversary` - caught: `dup-0089`
- `src/conductor.rs:31744-31861` `speculation_lenses_restore_a_gate_deleted_worktree_without_racing_and_stamp_the_winner_sha` - no duplicate found by reading
- `src/config.rs:845-854` `resolve_wall_clocks` - no duplicate found by reading
- `src/docs.rs:2012-2033` `diagnose_churn_carries_the_finding_audit_and_infra_separation` - no duplicate found by reading
- `src/eventstore/sqlite.rs:1458-1466` `prefix_upper_bound` - no duplicate found by reading
- `src/grounder/symbols/grounder.rs:776-804` `reindex_freshening_gate_skips_a_content_unchanged_file` - no duplicate found by reading
- `src/main.rs:11717-11734` `install_file_if_changed` - no duplicate found by reading
- `src/main.rs:18815-18832` `usage_text_names_the_build_cache_reset_mode` - no duplicate found by reading
- `src/main.rs:19646-19726` `migrate_project_identity_rekeys_the_graph_before_the_irreversible_stream_rename` - no duplicate found by reading
- `src/main.rs:22746-22773` `format_stats_spawn_timing_no_spawns_line_requires_both_empty_and_zero_unpaired` - no duplicate found by reading
- `src/main.rs:23500-23517` `parse_replay_args_requires_a_run_and_a_rev_in_either_order` - no duplicate found by reading
- `src/run.rs:255-283` `run_attribution` - no duplicate found by reading
- `src/spec.rs:1056-1080` `indented_sub_bullet_under_a_checkbox_is_flagged` - no duplicate found by reading
- `src/spec.rs:1158-1174` `spec_shape_advisories_ignores_length_added_by_a_continuation_line` - no duplicate found by reading
- `tests/architecture_integrity.rs:50-53` `architecture_text` - caught: `dup-0379`
- `tests/code_entity_test_exclusion_periphery.rs:957-998` `a_trailing_comment_on_cfg_test_still_excludes_the_module_through_the_public_api` - caught: `dup-0506`
- `tests/concepts_lens_view_periphery.rs:689-698` `page_script` - caught: `dup-0367`
- `tests/heartbeat_write_read_agree_periphery.rs:167-184` `seed_in_flight_spawn` - no duplicate found by reading
- `tests/reap_before_removal_audit.rs:469-497` `exemption_window` - no duplicate found by reading
...
- `tests/reset_derived_compaction_periphery.rs:1633-1699` `seed_run_history_and_duplication` - no duplicate found by reading
- `tests/simplification_audit.rs:7475-7478` `tok` - no duplicate found by reading
- `tests/simplification_audit.rs:10537-10615` `two_branches_each_adding_an_unrelated_function_to_a_different_file_never_perturb_an_existing_dead_code_entry` - caught: `dup-0707`
- `tests/spawn_scratch_reap_authorized_root_periphery.rs:161-164` `cleanup` - caught: `dup-0302`
- `tests/validate_behind_the_tree_periphery.rs:139-145` `gitsemver_available` - caught: `dup-0258`
- `tests/watchdog_cli_periphery.rs:44-51` `temp_project` - caught: `dup-0404`
- `tests/watchdog_cli_periphery.rs:573-654` `watch_once_reports_the_criterions_own_multi_anomaly_scenario_through_the_real_compiled_binary` - caught: `dup-0764`
- `tests/worker_persona_label_periphery.rs:241-253` `a_gap_18_respawn_id_still_renders_the_full_persona_led_label` - caught: `dup-0766`
