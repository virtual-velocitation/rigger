...
798 clusters (3861 total sites) across `src/` and `tests/`, found by `tests/simplification_audit.rs`'s deterministic normalized-token-shingle Jaccard pass (8-token shingles, threshold 0.72) plus five mandatory mechanical sweeps. Strict definition (spec 85 Goal): any logic present in more than one place anywhere in the codebase is a violation, with no "small enough to duplicate" exemption.

### Mandatory sweeps

- **Command::new call sites**: 391 site(s) - `dup-0006`
...
### Clusters (255 exact, 459 near, 84 semantic)
...
mandatory sweep: Command::new call sites - 391 site(s), collected mechanically regardless of the Jaccard pass (spec 85 Design)
...
Recall check (spec 85 THOROUGHNESS): 30 functions drawn by seeded random index (seed `85072026`, `sample_indices` over all 6898 functions scanned in `src/` and `tests/`, excluding `tests/prioritized_plan_citation_periphery.rs` - criterion 4's own citation-guard periphery test, whose function count grows as its citation-drift-guard mechanism hardens round over round; excluding it keeps that unrelated growth from ever reshuffling this already-verified draw), each read by hand - together with its host file's surrounding context, since a duplicate can live anywhere in the file or a sibling file - to judge whether a duplicate exists that the mechanical pass and the five sweeps above did not already catch.

- `src/community.rs:571-606` `graph` - no duplicate found by reading
- `src/conductor.rs:7069-7235` `plan_critique_loop` - no duplicate found by reading
- `src/conductor.rs:13209-13233` `prior_criterion_unit_integration_of_one_criterion_never_masks_an_abandoned_sibling_criterion_sharing_the_same_id` - no duplicate found by reading
- `src/conductor.rs:19999-20005` `subscribe_all` - caught: `dup-0066`
...
- `src/conductor.rs:41593-41600` `count` - caught: `dup-0050`
...
- `src/main.rs:17462-17473` `dir_size_bytes_sums_files_recursively` - no duplicate found by reading
- `src/metrics.rs:521-523` `gate_total` - no duplicate found by reading
- `src/safety.rs:53-57` `budget` - no duplicate found by reading
- `src/worktree.rs:4662-4700` `sweep_terminal_removes_merged_worktrees_and_keeps_inflight_ones` - caught: `dup-0375`
- `tests/canary_tolerant_attribution_periphery.rs:39-85` `spawn` - no duplicate found by reading
- `tests/cause_wire_periphery.rs:103-116` `seed_run_events` - caught: `dup-0426`
- `tests/cli.rs:9621-9669` `run_teardown_reclaims_run_level_scratch_at_a_definition_drift_halt` - caught: `dup-0476`
...
- `tests/cli.rs:18813-18825` `status_prints_no_dashboard_line_when_none_was_ever_recorded` - no duplicate found by reading
...
- `tests/graph_show_periphery.rs:975-1027` `graph_show_degrades_when_the_moved_name_is_ambiguous_in_the_file` - caught: `dup-0641`
...
- `tests/rationale_overlay_seam.rs:150-206` `the_served_explain_overlay_reads_the_lazy_whole_graph_not_the_state_poll` - no duplicate found by reading
- `tests/readable_graph_density_scaled_spacing.rs:50-56` `node_available` - caught: `dup-0269`
- `tests/reset_build_cache_periphery.rs:63-65` `shared_cache_dir` - caught: `dup-0698`
- `tests/responsibility_map_contract_periphery.rs:294-311` `the_committed_map_and_its_lines_sibling_are_position_joined` - no duplicate found by reading
...
- `tests/simplification_audit.rs:3231-3247` `dup_cluster_wire` - caught: `dup-0721`
- `tests/simplification_audit.rs:6428-6436` `two_sibling_functions_are_both_found_in_order` - no duplicate found by reading
- `tests/subject_view_memory_rail_client.rs:34-40` `node_available` - caught: `dup-0269`
