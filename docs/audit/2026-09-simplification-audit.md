...
775 clusters (3729 total sites) across `src/` and `tests/`, found by `tests/simplification_audit.rs`'s deterministic normalized-token-shingle Jaccard pass (8-token shingles, threshold 0.72) plus five mandatory mechanical sweeps. Strict definition (spec 85 Goal): any logic present in more than one place anywhere in the codebase is a violation, with no "small enough to duplicate" exemption.
...
- **.rigger-path string literals**: 707 site(s) - `dup-0060`
- **error-shaping helper functions**: 8 site(s) - `dup-0079`

### Clusters (247 exact, 448 near, 80 semantic)
...
- `tests/cli.rs:29222-29222` `".rigger"`
...
- `tests/cli.rs:29289-29329` `docs_installs_the_operator_lookup_rule_text_into_the_shipped_skill_and_handbook`
...
- `tests/cli.rs:29041-29146` `mcp_survives_a_grounder_resolution_failure_and_still_serves_peers_and_graph`
...
Recall check (spec 85 THOROUGHNESS): 30 functions drawn by seeded random index (seed `85072026`, `sample_indices` over all 6724 functions scanned in `src/` and `tests/`, excluding `tests/prioritized_plan_citation_periphery.rs` - criterion 4's own citation-guard periphery test, whose function count grows as its citation-drift-guard mechanism hardens round over round; excluding it keeps that unrelated growth from ever reshuffling this already-verified draw), each read by hand - together with its host file's surrounding context, since a duplicate can live anywhere in the file or a sibling file - to judge whether a duplicate exists that the mechanical pass and the five sweeps above did not already catch.

- `src/conductor.rs:6825-6904` `build_dag_critique_prompt` - no duplicate found by reading
- `src/conductor.rs:9349-9363` `record_merge_attempt` - caught: `dup-0038`
- `src/conductor.rs:14797-14799` `baseline_id` - no duplicate found by reading
- `src/conductor.rs:19801-19808` `read_all` - caught: `dup-0064`
- `src/conductor.rs:21146-21156` `worktree_registered_on` - no duplicate found by reading
- `src/conductor.rs:25380-25447` `speculation_exhaustive_integrate_door_red_blocks_a_candidate` - no duplicate found by reading
- `src/dash.rs:3519-3605` `calls_view` - no duplicate found by reading
- `src/dash.rs:8364-8411` `explain_returns_a_nodes_incident_edges_as_source_and_tier_tagged_provenance` - no duplicate found by reading
- `src/grounder/symbols/events.rs:441-457` `set_fresh` - no duplicate found by reading
- `src/main.rs:2256-2276` `acquire_step_lock` - no duplicate found by reading
- `src/main.rs:16249-16258` `behind_the_tree_message_is_silent_on_an_undecidable_or_zero_distance` - caught: `dup-0260`
- `src/main.rs:23125-23165` `format_stats_surfaces_spawn_timing_and_reports_unpaired_separately` - no duplicate found by reading
- `src/mcpserver.rs:30-35` `internal` - caught: `dup-0290`
...
- `tests/change_path_revert_periphery.rs:58-72` `run_rigger` - caught: `dup-0420`
- `tests/cli.rs:4994-5018` `write_reviewless_git_unit_workflow` - caught: `dup-0450`
- `tests/cli.rs:27822-27870` `describe_held_port_public_contract_holds_at_the_crate_boundary` - no duplicate found by reading
- `tests/community_resolution_knob.rs:627-674` `an_empty_rerun_keeps_the_last_good_assignment` - no duplicate found by reading
- `tests/gitsemver_path_inclusion_accounting_periphery.rs:78-80` `is_real_path_attribute_line` - no duplicate found by reading
...
- `tests/plan_stage_commit_landing_periphery.rs:2148-2155` `read_stream` - no duplicate found by reading
- `tests/projections_stay_local.rs:161-191` `start_kurrentdb` - caught: `dup-0665`
- `tests/simplification_audit.rs:9974-9996` `a_bare_call_in_the_same_file_as_its_definition_attributes_locally_with_no_import_needed` - caught: `dup-0734`
- `tests/spec_lint.rs:256-289` `validate_spec_does_not_misread_neither_or_as_either_or` - caught: `dup-0737`
- `tests/step_attention_periphery.rs:433-456` `marker_path_for_wave_item` - no duplicate found by reading
...
- `tests/store_resolution_cli.rs:160-184` `an_empty_kurrentdb_conn_is_treated_as_unset_not_a_server_with_no_address` - caught: `dup-0754`
