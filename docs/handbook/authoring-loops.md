...
6. **Criteria that share a blast radius belong in ONE unit** (or an explicitly sequenced chain). Splitting coupled edits to the same function across parallel units makes every reviewer see its siblings' half-landed changes, and the rejections compound: the spec-05 run put three setup-path criteria in three units and spent 40 review rejections and five escalations discovering they could not converge independently.
