...
- [x] Three-tier review: lenses -> adversary (refutes the lenses) -> adjudicator; the neutral adjudicator's verdict gates (§3.2)
- [x] Review is PER UNIT: each unit implements -> gates -> three-tier-reviews ITSELF -> integrates in one lifecycle; `defaults.review` panel inherited by every unit (incl. planner-proposed); reject/gate-fail loops that unit (§3.2)
