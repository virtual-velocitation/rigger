...
> Status: the core is built and runnable. The event store (SQLite by default, KurrentDB optional - both proven against one shared contract suite), the bi-temporal context graph, per-project segregation, the config loader (agent files + workflow YAML), the gate/autonomy/safety rails, the event-sourced run ledger, the conductor, the CLI agent driver, and the `rigger` binary are all implemented and CI-green. Agents run isolated in git worktrees (with their gates), independent stages run concurrently, and the live-awareness side-car and a grep grounder are in. The two deliberate not-yet pieces are the optional in-Claude-Code workflow driver and a vector (semantic) grounder; the CLI driver and the grep grounder cover the defaults. The full blueprint is [docs/architecture.md](docs/architecture.md).
