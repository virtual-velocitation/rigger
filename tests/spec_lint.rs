...
fn spec_lint_self_clean_over_the_committed_corpus() {
    ...
    assert_eq!(
        ...
        "F1 ownership's corpus-wide total is pinned to sdet's round-5 independently \
         ...
         (operator review 2026-09-11: same class), plus 6 hits added by the Mission Control \
         set specs/93 through specs/98 (operator review 2026-09-15: the same closing-checkbox \
         class, one each; every other criterion in those six carries an OWNS sentence), reviewed \
