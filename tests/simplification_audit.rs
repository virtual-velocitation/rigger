...
fn render_section_3() -> String {
    ...
    out.push_str(
        ...
        duplicate pair (`dup-0214`: `src/grounder/symbols/events.rs:36-38` and \
...
pub(crate) fn render_section_5() -> String {
    ...
    out.push_str(
        ...
        candidate class. Beyond the four in 5.2, the widest are: `dup-0358` \
        ...
        15 sites); `dup-0385` (a companion, 17-file/17-site variant of 5.2's \
        `run_stream_identity` fixture, alongside `dup-0386`'s 19-file version); \
        `dup-0414` (`write_two_stage_workflow` / \
        ...
        `dup-0387`/`dup-0388` (`seed_run_events`, an event-seeding helper, 7-9 \
        files); `dup-0478` (`apply_def_json` / `apply_ref_fresh`-shaped \
        fold-application helpers, 5 files); `dup-0485` (`community` / `concept` / \
        `def`-named single-field constructor helpers, 6 files); `dup-0701` \
    ...
    out.push_str(
        ...
        candidate class. The single largest anywhere in the suite: `dup-0679` (near, \
        ...
        `dup-0620`/`dup-0622` (15+7 sites, `tests/reap_before_removal_audit.rs`, \
...
fn render_section_6() -> String {
    ...
    out.push_str(
        "#### 10. Consolidate the 662 `.rigger`-path string-literal sites (`dup-0055`) - the \
    ...
    out.push_str(
        "#### 13. Consolidate the 6 error-shaping helper sites (`dup-0074`) - caution, \
    ...
    out.push_str(
        ...
        three test files named in `docs/audit/duplication-catalog.json` under `dup-0074`.\n\
    ...
    out.push_str(
        "- Scope: `page_script` (`dup-0354`, 19 files), `node_available` (`dup-0250`, 23 \
        files - merged with two related availability-check helpers), `temp_project` \
        (`dup-0380`, 20 files) and `run_stream_identity` (`dup-0386`, 19 files) - roughly 81 \
    ...
    out.push_str(
        "- Scope, largest first: `dup-0679` (42 sites, `tests/spec_lint.rs`), \
        `dup-0620`/`dup-0622` (15+7 sites, `tests/reap_before_removal_audit.rs`), \
        `dup-0609`/`dup-0610` (11+4 sites, `tests/no_os_kill_audit.rs`), \
        `dup-0653` (11 sites, `tests/simplification_audit.rs` - this very \
    ...
    out.push_str(
        ...
        `docs/audit/duplication-catalog.json`. Includes `dup-0612` (4 sites, \
    ...
    out.push_str(
        ...
        above) and 7 are the named tier-1/tier-4 items (`dup-0006`, `dup-0055`, `dup-0117`, \
        `dup-0139`, `dup-0140`, `dup-0214`, `dup-0074`); the remaining 327 clusters touching \
    ...
    match (file, line) {
        ...
        ("src/worktree.rs", 515) => (
            ...
            "is_dirty has no production caller - one of spec 87's own two Goal-cited worked \
             ...
             tree: its 3 references are all test-only. Line shifted 508->515 as spec 88's two \
             worktree units landed together (criterion 1 round 4: `MergeOutcome` inserted above \
             it in the same impl block, offset by the removal of the pre-round-4 \
             `IntegrateOutcome` enum/`expect_merged` impl entirely, both now test-scoped; \
             criterion 4: `CherryPickOutcome` inserted above it) - the same drift class \
             u88c1-audit-pin-repair and u88c4 both diagnosed for their own insertions; content \
             and disposition unchanged (see specs/90 criterion 2, not yet landed, for the \
             line-free fix this pin dance works around). expect_merged itself (formerly \
             src/worktree.rs:86) is no longer a candidate at all: round 4 moved it, together \
             with `IntegrateOutcome` and the pre-round-4 `integrate` method, into this file's \
             own `#[cfg(test)] mod tests` (a test-only recomposition of the newly-split \
             `merge_into_worktree`/`land`, since production - `integrate_and_emit` - now calls \
             those two split methods directly for its own row-level durable recording and has \
             no caller left for the combined form) - a test-scoped item is not a production \
...
mod tests {
    ...
    fn render_section_6_cites_every_tier_and_the_explicit_none_needed_category() {
        ...
        assert!(rendered.contains("dup-0117"));
        assert!(rendered.contains("dup-0139"));
        assert!(rendered.contains("dup-0140"));
        assert!(rendered.contains("dup-0074"));
        ...
        assert!(rendered.contains("dup-0679"));
