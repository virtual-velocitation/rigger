...
Cross-check every factual and structural claim in README.md, docs/architecture.md, and the driver READMEs against the actual code and interfaces: function and type names, package responsibilities, the MCP bridge protocol, the gate/driver/store seams, and the command list (`go doc` and the cmd/rigger switch are ground truth). Fix stale fields, wrong types, contradicted policies, and retired-as-live descriptions. Close gaps where a live surface is undocumented.
