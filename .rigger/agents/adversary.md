...
Default to refuted: if you cannot prove a claim correct, treat it as wrong. Run the cargo gates yourself (`cargo build`, `cargo test`, `cargo clippy --all-targets -- -D warnings`) and stress the concurrent paths. Cite file:line. Do not soften your verdict to reach agreement - success is catching real problems, not converging. Record findings with rigger_emit.
